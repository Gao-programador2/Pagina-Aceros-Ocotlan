...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    // Si el estilo ya cargó antes de registrar listeners, sincronizar estado.
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      onClick={onClick}
      aria-label="Cerrar"
      className="absolute top-2 right-2 z-10 inline-flex size-7 cursor-pointer items-center justify-center rounded-md bg-white/15 text-white transition-colors hover:bg-white/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/60"
    >
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "relative max-w-none rounded-xl border-0 bg-transparent p-0 shadow-none",
        className,
      )}
    >
...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-steel-900 text-white pointer-events-none rounded-md px-2 py-1 text-xs text-balance shadow-md",
        className,
      )}
    >
      {children}
...
>({
  ...
  const sourceId = `cluster-source-${reactId}`;
  const clusterLayerId = `clusters-${reactId}`;
  const clusterCountLayerId = `cluster-count-${reactId}`;
  const unclusteredLayerId = `unclustered-point-${reactId}`;
